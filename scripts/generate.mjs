...
const categorizedEmojis = Object.entries(emojis).reduce((acc, [name, url]) => {
  const matchingCategories = Object.entries(categories).filter(([, regexes]) =>
...
await Promise.all([
  ...
  ...Object.entries(categorizedEmojis).map(([category, emojis]) =>
    writeMd(`categorized/${category}.md`, partition(emojis)),
  ),
  ...Object.entries(byPrefixEmojis).map(([prefix, emojis]) =>
    writeMd(`by-prefix/${prefix}.md`, partition(emojis)),
  ),
]);
